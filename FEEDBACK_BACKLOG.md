...
- Resolution: The sharing dialog now labels Discord access as editor access, states that editors can change the plan, distinguishes it from the view-only link, disables empty submissions, and reports success or failure.
- Open question: Commenter access remains a possible future role, tracked by 7.2.
- Verification: `npm run check`; `npm run build`; production root and deployed bundle HTTP 200; live bundle contains the new editor-access copy.
- Deployment: production version `e800af49-bd1b-4d38-b5e4-218469e0da6d`.
